...
@integration_bp.route('/sales/insert_update', methods=['POST'])
def insert_or_update_sale():
    try:
        ...
        cursor.execute("""
            ...
            WHERE agent_id = %s AND product_id = %s
        """, (agent_id, product_id))
        if cursor.fetchone() is None:
            return jsonify({
                "message": f"Agent '{agent_id}' is not permitted to sell product '{product_id}'."
            }), 403
        ...
        cursor.execute("""
            ...
            SET sale_date = %s,
                ...
                amount = %s
            WHERE sale_id = %s
        """, (sale_date, agent_id, product_id, amount, sale_id))

        if cursor.rowcount == 0:
            cursor.execute("""
                INSERT INTO tdb.sale (sale_id, sale_date, agent_id, product_id, amount)
                VALUES (%s, %s, %s, %s, %s)
            """, (sale_id, sale_date, agent_id, product_id, amount))
