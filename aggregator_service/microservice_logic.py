...
# ETL logic for dwh.dim_agent warehouse table
def load_dim_agent():
    try:
        # Connect to transactional database
        conn_trx = psycopg2.connect(
        ...
        # Insert into dwh.dim_agent with auto-incrementing agent_sk
        for sk, row in enumerate(rows, start=1):
            cursor_dwh.execute("""
                INSERT INTO dwh.dim_agent (agent_sk, agent_id, agent_name, team_name, branch_name)
                VALUES (%s, %s, %s, %s, %s)
            """, (sk, row[0], row[1], row[2], row[3]))
...
# ETL logic for dwh.fact_sales warehouse table
def load_fact_sales():
    try:
        # Connect to transactional database
        conn_trx = psycopg2.connect(
        ...
        cursor_trx.execute("""
            SELECT sale_id, sale_date, agent_id, product_id, amount
            FROM tdb.sale
        """)
        ...
        cursor_dwh.execute("SELECT agent_id, agent_sk FROM dwh.dim_agent")
        agent_map = {row[0]: row[1] for row in cursor_dwh.fetchall()}

        # Insert into dwh.fact_sales using surrogate keys
        for sale in sales:
            sale_id, sale_date, agent_id, product_id, amount = sale
            date_sk = date_map.get(sale_date)
            product_sk = product_map.get(product_id)
            agent_sk = agent_map.get(agent_id)
...
# Load dwh.sales_team_aggr aggregated sales table
def load_sales_team_aggr():
    try:
        ...
        # Insert aggregated data into dwh.sales_team_aggr
...
def load_sales_branch_aggr():
    try:
        ...
        # Insert aggregated data into dwh.sales_branch_aggr
...
# Load dwh.sales_product_aggr aggregated sales table
